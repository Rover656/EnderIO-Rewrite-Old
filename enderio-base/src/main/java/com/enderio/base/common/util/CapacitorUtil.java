...
public class CapacitorUtil {
    ...
    public static Optional<ICapacitorData> getCapacitorData(ItemStack itemStack, Level level) {
        ...
        Optional<CapacitorDataRecipe> recipe = level.getRecipeManager().getAllRecipesFor(EIORecipes.Types.CAPACITOR_DATA).stream().filter(r -> r.matches(itemStack)).findFirst();
