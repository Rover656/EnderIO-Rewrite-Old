...
public class AutoSmeltHandler {
	...
	public static void handleBlockBreak(BlockEvent.BreakEvent event) {
		if (!event.getWorld().isClientSide() && EnchantmentHelper.getItemEnchantmentLevel(EIOEnchantments.AUTO_SMELT.get(), event.getPlayer().getMainHandItem()) > 0) { // Checks if running on server and enchant is on tool
			...
			LootContext.Builder lootcontext$builder = (new LootContext.Builder(serverWorld)
					...
					.withParameter(LootContextParams.TOOL, event.getPlayer().getMainHandItem())); // Makes lootcontext to calculate drops
			List<ItemStack> drops = event.getState().getDrops(lootcontext$builder); // Calculates drops
			for (ItemStack item : drops) { // Iteration
				ItemStack stack = serverWorld.getRecipeManager().getRecipeFor(RecipeType.SMELTING, new SimpleContainer(item), serverWorld)
